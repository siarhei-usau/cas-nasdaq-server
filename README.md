...
The `/etc/cas/config` directory contains the configuration files.
...
- Use the password `changeit` for both the keystore and the key/certificate entries. For example: `"%JAVA_HOME%\bin\keytool" -genkey -alias tomcat -keyalg RSA -keystore d:\etc\cas\thekeystore -storepass changeit -keypass changeit -dname "cn=localhost, ou=tomcat, o=nasdaq, c=US"`
...
Please use CAS login/password: test/test
